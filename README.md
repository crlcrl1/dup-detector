...
| Type-3 | Near-miss clones with a few statements added or removed    | Yes (opt-in)      |
...
- **Token-efficient responses** — file paths + line ranges + size + similarity, no source excerpts.
...
file discovery (.gitignore aware)
  ...
  -> maximal match extension    (gap-tolerant when Type-3 is enabled)
...
5. **Type-3 (opt-in).** Extension tolerates small gaps (up to a few tokens per side) by looking for a resumption run; candidate spans are scored with an LCS over parameterized keys and filtered by a similarity threshold.
...
# include Type-3 near-miss clones
...
| `--type3`                 | off     | Detect Type-3 near-miss clones               |
...
| `find_clones_for_region` | "Is the code I'm writing duplicated?" | `file`, `start_line`, `end_line`, `scope?`, `min_tokens?`, `max_groups?`, `type3?`                       |
| `reindex`                | Rebuild the in-memory index           | `path?`                                                                                                  |

- `scope` defaults to the current working directory.
- `types` accepts `"type-1"`, `"type-2"`, `"type-3"`.
...
src/
  ...
  lib.rs        library root
  config.rs     Config (thresholds, language toggles, Type-3 knobs)
  ...
  detect.rs     seed-and-extend, bijection, gap-tolerant extension, clustering
...
- Phases 0–4, 6 done: tokens, encoding, detection, Type-1/2/3, corpus regression.
