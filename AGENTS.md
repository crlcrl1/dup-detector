...
Per project, one `dup-detector.toml` at the project root (constant `config::CONFIG_FILE_NAME`). It is read at startup by searching upwards from the starting path to the filesystem root: `scan` starts at the scanned path, `mcp` starts at the server's working directory, and the server also reads each scope/index root and falls back to the startup config when a root has none. The `mcp` and `lsp` servers re-read the file when its mtime changes (via `Config::source_path`/`load_from_file`) and rebuild the affected index. All keys are optional; a missing file yields `Config::default()`. Unknown keys, unknown language names, and degenerate values (`0` for `min_lines`/`min_occurrences`/`max_bucket`/`seed_window`/`max_file_bytes`, or `max_file_bytes` above 4 GiB) are errors (`ConfigError`). Hidden (dot) directories are skipped unless `include_hidden = true`. CLI flags and MCP tool params override the file via `with_limits`/`with_max_file_bytes` (both validate and return `Result`) or direct field assignment. Schema: `min_lines`, `min_occurrences`, `max_bucket`, `seed_window`, `max_groups`, `parameterize_literals`, `languages` (array of language names accepted by `LanguageId::from_name`), `no_ignore` (default off; also scan files excluded by .gitignore/.ignore rules), `include_hidden` (default off; hidden dot-directories are otherwise skipped like ripgrep), `max_file_bytes` (default 2 MiB; larger files are skipped by the index and by LSP overlays to bound parse memory), `filter_boilerplate` (default on; drop clone groups whose occurrences contain no logic markers).

### LSP server

`lsp.rs` implements `tower_lsp::LanguageServer` over stdio (`dup-detector lsp`). It publishes clone occurrences in open documents as `Warning` diagnostics and answers `textDocument/definition`, `textDocument/references` and `textDocument/hover` from the last analysis snapshot. Editor buffers (including unsaved edits) are overlaid on the disk index by tokenizing the changed document in memory. Efficiency rules, keep them intact: incremental text sync (only the changed document is re-tokenized), a 350 ms debounce with coalescing and generation-based cancellation of stale runs, analysis on `spawn_blocking`, and incremental re-detection where `allowed` seeds are the changed regions plus the window signatures of the clones reported in the previous snapshot (so unchanged clones are reproduced and the candidate set scales with edits, not project size). A `PendingChanges` set tracks touched byte ranges; externally changed non-open files are treated as whole-file changes. Open documents of languages disabled by `languages` are ignored, mirroring the index, and the config file is re-read when its mtime changes. The index does mtime/size incremental refresh and diagnostics are capped per file.
